...
describe('Payment e2e test', () => {
  ...
  const paymentSample = { transactionId: 'boo until', amount: 22253.2, currency: 'mis', method: 'COD', status: 'SUCCESS' };
  ...
  describe('new Payment page', () => {
    ...
    it('should create an instance of Payment', () => {
      cy.get(`[data-cy="transactionId"]`).type('sneak cinema smoothly');
      cy.get(`[data-cy="transactionId"]`).should('have.value', 'sneak cinema smoothly');

      cy.get(`[data-cy="amount"]`).type('22173.28');
      cy.get(`[data-cy="amount"]`).should('have.value', '22173.28');

      cy.get(`[data-cy="currency"]`).type('wit');
      cy.get(`[data-cy="currency"]`).should('have.value', 'wit');

      cy.get(`[data-cy="method"]`).select('CREDIT_CARD');

      cy.get(`[data-cy="status"]`).select('PARTIAL_REFUND');

      cy.get(`[data-cy="gatewayResponse"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="gatewayResponse"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="referenceNumber"]`).type('accidentally anenst private');
      cy.get(`[data-cy="referenceNumber"]`).should('have.value', 'accidentally anenst private');

      cy.get(`[data-cy="failureReason"]`).type('really rule');
      cy.get(`[data-cy="failureReason"]`).should('have.value', 'really rule');
