...
describe('Cart e2e test', () => {
  ...
  describe('new Cart page', () => {
    ...
    it('should create an instance of Cart', () => {
      cy.get(`[data-cy="sessionId"]`).type('ack till coaxingly');
      cy.get(`[data-cy="sessionId"]`).should('have.value', 'ack till coaxingly');

      cy.get(`[data-cy="status"]`).select('CONVERTED');

      cy.get(`[data-cy="expiresAt"]`).type('2025-09-16T23:09');
      cy.get(`[data-cy="expiresAt"]`).blur();
      cy.get(`[data-cy="expiresAt"]`).should('have.value', '2025-09-16T23:09');
