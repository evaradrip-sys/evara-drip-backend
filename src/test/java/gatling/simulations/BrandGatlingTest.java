...
public class BrandGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all brands").get("/api/brands").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created brand").get("${new_brand_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created brand").delete("${new_brand_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
