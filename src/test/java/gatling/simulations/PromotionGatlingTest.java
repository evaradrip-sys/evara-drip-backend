 ...
 */
public class PromotionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all promotions").get("/api/promotions").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new promotion")
                        .post("/api/promotions")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created promotion").get("${new_promotion_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created promotion").delete("${new_promotion_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Promotion entity").exec(scn);
