...
public class CartItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cartItems").get("/api/cart-items").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cartItem")
                        .post("/api/cart-items")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"addedPrice\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cartItem").get("${new_cartItem_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created cartItem").delete("${new_cartItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
