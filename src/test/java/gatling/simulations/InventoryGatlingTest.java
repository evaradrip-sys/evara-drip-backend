...
public class InventoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all inventories").get("/api/inventories").headers(headersHttpAuthenticated).check(status().is(200)))
                    ...
                    http("Create new inventory")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"quantity\": 0" +
                                ", \"reservedQuantity\": 0" +
                                ", \"warehouse\": \"SAMPLE_TEXT\"" +
                                ", \"lastRestocked\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"reorderLevel\": 0" +
                                ", \"reorderQuantity\": 0" +
                                "}"
                            )
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created inventory").get("${new_inventory_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created inventory").delete("${new_inventory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
