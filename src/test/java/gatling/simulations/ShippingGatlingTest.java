 ...
 */
public class ShippingGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all shippings").get("/api/shippings").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new shipping")
                        .post("/api/shippings")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"estimatedDelivery\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"actualDelivery\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"shippingCost\": 0" +
                                ", \"status\": \"PENDING\"" +
                                ", \"notes\": null" +
                                "}"
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_shipping_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created shipping").get("${new_shipping_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created shipping").delete("${new_shipping_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Shipping entity").exec(scn);
