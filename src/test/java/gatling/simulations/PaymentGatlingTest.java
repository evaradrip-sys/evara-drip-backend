...
public class PaymentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all payments").get("/api/payments").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created payment").get("${new_payment_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created payment").delete("${new_payment_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
