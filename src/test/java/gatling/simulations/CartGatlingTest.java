...
public class CartGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all carts").get("/api/carts").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cart").get("${new_cart_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created cart").delete("${new_cart_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
