 ...
 */
public class WishlistGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all wishlists").get("/api/wishlists").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new wishlist")
                        .post("/api/wishlists")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"priority\": 0" + ", \"notes\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_wishlist_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created wishlist").get("${new_wishlist_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created wishlist").delete("${new_wishlist_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Wishlist entity").exec(scn);
