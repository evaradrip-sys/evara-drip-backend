 ...
 */
public class ReviewGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reviews").get("/api/reviews").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new review")
                        .post("/api/reviews")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"status\": \"PENDING\"" +
                                ", \"response\": null" +
                                ", \"responseDate\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created review").get("${new_review_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created review").delete("${new_review_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Review entity").exec(scn);
