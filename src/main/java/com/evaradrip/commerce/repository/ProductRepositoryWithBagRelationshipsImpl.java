...
public class ProductRepositoryWithBagRelationshipsImpl implements ProductRepositoryWithBagRelationships {
    ...
    public Optional<Product> fetchBagRelationships(Optional<Product> product) {
        return product.map(this::fetchPromotions);
    }
    ...
    public List<Product> fetchBagRelationships(List<Product> products) {
        return Optional.of(products).map(this::fetchPromotions).orElse(Collections.emptyList());
    }

    Product fetchPromotions(Product result) {
        return entityManager
            .createQuery("select product from Product product left join fetch product.promotions where product.id = :id", Product.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Product> fetchPromotions(List<Product> products) {
        ...
        List<Product> result = entityManager
            .createQuery("select product from Product product left join fetch product.promotions where product in :products", Product.class)
