...
public interface ReviewRepository extends JpaRepository<Review, Long>, JpaSpecificationExecutor<Review> {
    ...
    @Query(
        value = "select review from Review review left join fetch review.user left join fetch review.product",
        countQuery = "select count(review) from Review review"
    )
    Page<Review> findAllWithToOneRelationships(Pageable pageable);

    @Query("select review from Review review left join fetch review.user left join fetch review.product")
    List<Review> findAllWithToOneRelationships();

    @Query("select review from Review review left join fetch review.user left join fetch review.product where review.id =:id")
