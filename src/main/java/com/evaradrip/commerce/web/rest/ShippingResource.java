...
import com.evaradrip.commerce.service.ShippingQueryService;
...
/**
 * REST controller for managing {@link com.evaradrip.commerce.domain.Shipping}.
 */
@RestController
...
public class ShippingResource {
     ...
     */
    @PostMapping("")
    public ResponseEntity<ShippingDTO> createShipping(@Valid @RequestBody ShippingDTO shippingDTO) throws URISyntaxException {
    ...
    public ResponseEntity<List<ShippingDTO>> getAllShippings(
        ShippingCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get Shippings by criteria: {}", criteria);

        Page<ShippingDTO> page = shippingQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /shippings/count} : count all the shippings.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countShippings(ShippingCriteria criteria) {
        LOG.debug("REST request to count Shippings by criteria: {}", criteria);
        return ResponseEntity.ok().body(shippingQueryService.countByCriteria(criteria));
    }
