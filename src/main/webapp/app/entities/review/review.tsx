  ...
  return (
    <div>
      <h2 id="review-heading" data-cy="ReviewHeading">
        <Translate contentKey="evaradripApp.review.home.title">Reviews</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="evaradripApp.review.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/review/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="evaradripApp.review.home.createLabel">Create new Review</Translate>
          </Link>
        </div>
      </h2>
        ...
        {reviewList && reviewList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="evaradripApp.review.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('helpfulCount')}>
                  <Translate contentKey="evaradripApp.review.helpfulCount">Helpful Count</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('helpfulCount')} />
                </th>
                <th className="hand" onClick={sort('notHelpfulCount')}>
                  <Translate contentKey="evaradripApp.review.notHelpfulCount">Not Helpful Count</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('notHelpfulCount')} />
                </th>
                <th className="hand" onClick={sort('verifiedPurchase')}>
                  <Translate contentKey="evaradripApp.review.verifiedPurchase">Verified Purchase</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('verifiedPurchase')} />
                </th>
                ...
                <th className="hand" onClick={sort('response')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('response')} />
                </th>
                ...
                <th>
                  <Translate contentKey="evaradripApp.review.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="evaradripApp.review.product">Product</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {reviewList.map((review, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{review.verifiedPurchase ? 'true' : 'false'}</td>
                  <td>
                    <Translate contentKey={`evaradripApp.ReviewStatus.${review.status}`} />
                  </td>
                  <td>{review.response}</td>
                  <td>{review.responseDate ? <TextFormat type="date" value={review.responseDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{review.user ? review.user.login : ''}</td>
                  <td>{review.product ? <Link to={`/product/${review.product.id}`}>{review.product.name}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="evaradripApp.review.home.notFound">No Reviews found</Translate>
            </div>
          )
        )}
      </div>
